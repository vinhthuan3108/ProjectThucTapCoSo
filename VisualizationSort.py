...
class SortVisualizationWidget(QtWidgets.QWidget):
    ...
    def bubble_sort_visualization(self, data, ascending=True):
        """Bubble Sort với cập nhật biểu đồ"""
        n = len(data)
        sorted_flags = [False] * n  # Đánh dấu phần tử đã sắp xếp
        for i in range(n - 1):
            swapped = False
            for j in range(n - i - 1):
                # So sánh và hoán đổi (theo hướng tăng dần hoặc giảm dần)
            ...
            if not swapped:  # Nếu không có hoán đổi nào thì kết thúc
                break
    ...
    def merge_sort_visualization(self, data, ascending=True):
        """Merge Sort với cập nhật biểu đồ"""
    ...
    def merge_sort_step(self, data, left, right, color_array, ascending):
        """Phân chia mảng và gọi hàm merge"""
        if left < right:
            mid = (left + right) // 2
    ...
    def merge(self, data, left, mid, right, color_array, ascending):
        ...
        while i < len(left_copy) and j < len(right_copy):
            # Vẽ lại màu sắc khi đang hợp nhất
        ...
        # Vẽ lại biểu đồ sau khi hợp nhất
        self.draw_data(data, color_array)
        QtCore.QCoreApplication.processEvents()
        time.sleep(self.animation_speed)

    def sort_ascending(self):
        """Khởi chạy thuật toán sắp xếp tăng dần"""
    ...
    def sort_descending(self):
        """Khởi chạy thuật toán sắp xếp giảm dần"""
        if self.sort_type == "bubble":
